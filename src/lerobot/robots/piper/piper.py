...
class Piper(BaseRobot):
    ...
    def _set_joint_state(self, state: np.ndarray) -> None:
        ...
        # Convert numpy floats to integers before sending to Piper SDK
        joint_args = [int(x) for x in state[:6]]
        self.arm.JointCtrl(*joint_args)
    ...
    def _set_ee_state(self, state: np.ndarray) -> None:
        ...
        # Convert numpy floats to integers before sending to Piper SDK
        ee_args = [int(x) for x in state[:6]]
        self.arm.EndPoseCtrl(*ee_args)
        self.arm.GripperCtrl(int(state[6]), 1000, 0x01, 0)
